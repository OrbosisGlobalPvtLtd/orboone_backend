...
class EmployeeC extends Controller
{
    ...
    public function manageUpdate(Request $request, $employee)
    {
        ...
        try {
            ...
            $profileData = [
                'employee_id' => $employee,
                'date_of_birth' => $request->has('date_of_birth') ? $request->input('date_of_birth') : ($oldProfile->date_of_birth ?? null),
                'gender' => $request->has('gender') ? $request->input('gender') : ($oldProfile->gender ?? null),
                'address' => $request->has('address') ? $request->input('address') : ($oldProfile->address ?? null),
                'highest_qualification' => $request->has('highest_qualification') ? $request->input('highest_qualification') : ($oldProfile->highest_qualification ?? null),
                'cgpa_percentage' => $request->has('cgpa_percentage') ? $request->input('cgpa_percentage') : ($oldProfile->cgpa_percentage ?? null),
                'total_experience' => $request->has('total_experience') ? $request->input('total_experience') : ($oldProfile->total_experience ?? null),
                'emergency_contact_number' => $request->has('emergency_contact_number') ? $request->input('emergency_contact_number') : ($oldProfile->emergency_contact_number ?? null),
                'bank_account_no' => $request->has('bank_account_no') ? $request->input('bank_account_no') : ($oldProfile->bank_account_no ?? null),
                'bank_account_type' => $request->has('bank_account_type') ? $request->input('bank_account_type') : ($oldProfile->bank_account_type ?? null),
                'bank_holder_name' => $request->has('bank_holder_name') ? $request->input('bank_holder_name') : ($oldProfile->bank_holder_name ?? null),
                'ifsc_code' => $request->has('ifsc_code') ? ($request->input('ifsc_code') ? strtoupper($request->input('ifsc_code')) : null) : ($oldProfile->ifsc_code ?? null),
                'bank_branch' => $request->has('bank_branch') ? $request->input('bank_branch') : ($oldProfile->bank_branch ?? null),
    ...
    private function resolveShiftAndPolicyBySchedule(?string $schedule, string $workMode, string $employmentType): array
    {
        ...
        if ($config && isset($config[$scheduleKey])) {
            ...
            } elseif ($scheduleKey === 'part_time_morning') {
                $shiftCode = 'part_time_morning';
                $policyName = 'Part Time Morning Policy';
            } elseif ($scheduleKey === 'part_time_evening') {
                $shiftCode = 'part_time_evening';
                $policyName = 'Part Time Evening Policy';
            } else {
                $shiftCode = $scheduleKey;
                $policyName = ucwords(str_replace('_', ' ', $scheduleKey)) . ' Policy';
            }
        }
    ...
    {
        if ($schedule === 'flexible_part_time') {
    ...
    private function adjustWorkScheduleTypeForView($employeeData): void
    {
        ...
        if ($activePolicy) {
            ...
            if (str_contains($policyName, 'Flexible')) {
                $employeeData->work_schedule_type = 'flexible_part_time';
            } elseif (str_contains($policyName, 'Morning')) {
