...
<div class="container-fluid py-4 px-4">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        ...
        <div class="d-flex">
            <a href="{{ route('pages.add_task') }}" class="btn btn-orb mr-2">
                <i class="fas fa-plus mr-2"></i> Create Task
            </a>
            <a href="{{ route('pages.task_print') }}" class="btn btn-light shadow-sm" style="border-radius: 50px;" target="_blank">
    ...
    <div class="filter-bar">
        <form action="{{ route('task_management') }}" method="GET" class="row align-items-end">
            ...
            <div class="col-md-3">
                ...
                @if(request()->hasAny(['search', 'user_id', 'status', 'start_date', 'end_date']))
                    <a href="{{ route('task_management') }}" class="btn btn-link btn-sm text-danger mt-1">Clear Filters</a>
                @endif
            </div>
            @if(request('status'))
                ...
                <tbody>
                    @forelse ($tasks as $task)
                        <tr id="row-{{ $task->id }}">
                            ...
                            <td class="text-right">
                                <div class="btn-group">
                                    <a href="{{ route('pages.edit_task', $task->id) }}" class="btn btn-sm btn-light text-primary hover-shadow" title="Edit Task">
                                        <i class="fas fa-edit"></i>
                                    </a>
                                    <form action="{{ route('pages.delete_task', $task->id) }}" method="POST" id="deleteForm-{{ $task->id }}" class="d-inline">
                                        @csrf
                                        @method('DELETE')
