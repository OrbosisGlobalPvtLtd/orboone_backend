...
class LeaveApprovalC extends Controller
{
    ...
    {
        $requests = \App\Models\HRMS\Leave\LeaveApplicationM::with('employee.employeeDetail')
            ->latest()
            ->paginate(20);

        $accesses = \App\Models\Core\AccessM::where('role_id', auth()->user()->role_id)->get();

        return view('hrms.leave.approvals.index', compact('requests', 'accesses'))
    ...
    {
        $leaveApplication = \App\Models\HRMS\Leave\LeaveApplicationM::with('employee')->findOrFail($id);
        ...
        $allocation = \App\Models\HRMS\Leave\LeaveAllocationM::firstOrCreate(
    ...
    {
        $leaveApplication = \App\Models\HRMS\Leave\LeaveApplicationM::findOrFail($id);
