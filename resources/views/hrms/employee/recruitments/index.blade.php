      ...
      <div class="bg-light text-dark card p-3 overflow-auto">
        <div class="d-flex justify-content-between">
          @if (collect($accesses)->where('menu_id', 7)->first()->status == 2)
            <a href="{{ route('hrms.recruitments.create') }}" class="btn btn-outline-dark mb-3 w-25">
              <i class="fas fa-plus mr-1"></i>
                <span> Create</span>
            </a>
          @endif
          <a href="{{ route('hrms.recruitments.export') }}" class="btn btn-outline-dark mb-3 w-25" target="_blank">
        ...
        <table class="table table-light table-striped table-hover table-bordered text-center">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="w-25"><a href="{{ route('hrms.recruitments.show', ['recruitment' => $recruitment->id ]) }}">{{ $recruitment->title }}</a></td>
