...
<head>
    ...
    <title>Set Password | {{ $branding['company_name'] ?? config('app.name', 'OrboOne HRMS') }}</title>
    ...
    <style>
        ...
        .alert{border-radius:12px;padding:10px 12px;margin-bottom:14px;font-size:13px;font-weight:700;background:#FEF3F2;color:#B42318;}
    </style>
</head>
<body>
    <div class="auth-card">
        <h1>Set Your Password</h1>
        <p>{{ $user->email }} can set a new password for web and mobile login.</p>

        @if($errors->any())<div class="alert">{{ $errors->first() }}</div>@endif

        <form action="{{ route('employee.password.setup.update', $token) }}" method="POST">
            @csrf
            <label>New Password</label>
            <input type="password" name="password" required>
            <label>Confirm Password</label>
            <input type="password" name="password_confirmation" required>
            <button type="submit"><i class="fas fa-lock"></i> Set Password</button>
        </form>
    </div>
</body>
</html>
