    ...
    public function updateProfile(Request $request)
    {
        try {
            ...
            $profile->update([
                ...
                'date_of_birth'         => $request->filled('date_of_birth') ? $request->date_of_birth : $profile->date_of_birth,
                'gender'                => $request->filled('gender') ? $request->gender : $profile->gender,
                'address'               => $request->filled('address') ? $request->address : $profile->address,
                'highest_qualification' => $request->filled('highest_qualification') ? $request->highest_qualification : $profile->highest_qualification,
                'cgpa_percentage'       => $request->filled('cgpa_percentage') ? $request->cgpa_percentage : $profile->cgpa_percentage,
                'total_experience'      => $request->filled('total_experience') ? $request->total_experience : $profile->total_experience,
                'resume_file'           => $resumeFilePath,
                'bank_account_no'       => $request->filled('bank_account_no') ? $request->bank_account_no : $profile->bank_account_no,
                'bank_account_type'     => $request->filled('bank_account_type') ? $request->bank_account_type : $profile->bank_account_type,
                'bank_holder_name'      => $request->filled('bank_holder_name') ? $request->bank_holder_name : $profile->bank_holder_name,
                'ifsc_code'             => $request->filled('ifsc_code') ? $request->ifsc_code : $profile->ifsc_code,
                'bank_branch'           => $request->filled('bank_branch') ? $request->bank_branch : $profile->bank_branch,
