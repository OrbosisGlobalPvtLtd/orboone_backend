...
class EmployeeDocumentsC extends Controller
{
    ...
    public function index()
    {
        ...
        $documentTypes = \App\Models\HRMS\Document\DocumentTypeM::where('scope', 'employee')->get();

        return view('hrms.document.employee_documents.index', compact('employees', 'allEmployees', 'documentTypes'));
    }
    ...
    public function show(Employee $employee)
    {
        ...
        return view('hrms.document.employee_documents.show', compact('employee', 'documents', 'documentTypes'));
    }
    ...
    public function store(Request $request, Employee $employee)
    {
        ...
        $type = \App\Models\HRMS\Document\DocumentTypeM::findOrFail($request->document_type_id);

        $upload = function ($key) use ($request, $employee) {
            if ($request->hasFile($key)) {
                return $this->documentService->storeEmployeeDocumentFile(
                    $request->file($key),
                    (int) $employee->id,
                    $key
                );
