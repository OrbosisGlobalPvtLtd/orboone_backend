...
use App\Models\EmployeeClaimApproval;
use App\Models\EmployeeClaimDocument;
use App\Models\EmployeeReimbursement;
use App\Models\EmployeeReimbursementApproval;
use App\Models\EmployeeReimbursementDocument;
use App\Models\PayrollComponent;
use App\Models\PayrollComponentType;
use App\Models\EmployeePayroll;
use App\Models\EmployeePayrollDetail;
use App\Models\EmployeePayrollApproval;
...
class ApiController extends Controller
{
      ...
      public function updateProfile(Request $request)
    {
        try {
            ...
            $detail = \App\Models\EmployeeDetail::where('employee_id', $employee->id)->first();
            ...
            if ($detail) {
                ...
                $detail = \App\Models\EmployeeDetail::create($detailChanges);
            }
    ...
    {
        return \App\Models\LeaveApplication::where('employee_id', $employeeId)
    ...
    {
        return response()->json(['status' => true, 'data' => \App\Models\LeaveType::orderBy('name')->get()], 200);
    }
    ...
    public function getLeaveBalance()
    {
        try {
            ...
            // ── 1. Allocation (quota) ──────────────────────────────────────
            $alloc   = \App\Models\LeaveAllocation::where('employee_id', $employee->id)->where('year', $year)->first();
            ...
            $usedPl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'PL')->where('status', 'approved')
                ->whereYear('start_date', $year)->sum('total_days');

            $usedSl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'SL')->where('status', 'approved')
                ->whereYear('start_date', $year)->sum('total_days');

            $lwpDirect = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'LWP')->where('status', 'approved')
                ->whereYear('start_date', $year)->sum('total_days');

            $lwpEmbedded = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
            ...
            $pendingPl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'PL')->where('status', 'pending')
                ->whereYear('start_date', $year)->sum('total_days');

            $pendingSl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'SL')->where('status', 'pending')
                ->whereYear('start_date', $year)->sum('total_days');

            // ── 5. Monthly usage breakdown ─────────────────────────────────
            $monthlyPl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
                ->where('leave_type', 'PL')->whereIn('status', ['approved', 'pending'])
                ->whereYear('start_date', $year)->whereMonth('start_date', $month)->sum('total_days');

            $monthlySl = (float) \App\Models\LeaveApplication::where('employee_id', $employee->id)
    ...
    public function applyLeave(Request $request)
    {
        try {
            ...
            $allocation     = \App\Models\LeaveAllocation::where('employee_id', $employee->id)->where('year', $year)->first();
    ...
    public function myLeaves()
    {
        try {
            ...
            $history = \App\Models\LeaveApplication::where('employee_id', $employee->id)
    ...
    public function cancelLeaveRequest($id)
    {
        try {
            ...
            $application = \App\Models\LeaveApplication::where('id', $id)->where('employee_id', $employee->id)->first();
    ...
    {
        try {
            $application = \App\Models\LeaveApplication::with('employee')->find($id);
            if (!$application) return response()->json(['status' => false, 'message' => 'Leave application not found.'], 404);
            ...
            $allocation = \App\Models\LeaveAllocation::where('employee_id', $application->employee_id)
    ...
    {
        try {
            $application = \App\Models\LeaveApplication::find($id);
            if (!$application) return response()->json(['status' => false, 'message' => 'Leave application not found.'], 404);
            if ($application->status !== 'pending') return response()->json(['status' => false, 'message' => "Leave is already '{$application->status}'."], 400);
    ...
    public function myLeaveCalendar(Request $request)
    {
        try {
            ...
            $holidays = \App\Models\NationalHoliday::whereYear('holiday_date', $year)
    ...
    public function createEmployee(Request $request)
    {
        ...
        $allocationController = new \App\Http\Controllers\LeaveAllocationController();
        ...
        try {
            ...
            return DB::transaction(function () use ($request, $user, $employee, $uploadFile) {
                ...
                if ($request->filled('document_type_id')) {
                    $docType = \App\Models\DocumentTypeModal::find($request->document_type_id);
